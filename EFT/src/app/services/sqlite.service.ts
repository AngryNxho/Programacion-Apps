...
export class SqliteService {
  ...
  async getListasUsuario(userId: number) {
    const sql = `
      SELECT l.*, COUNT(lb.id) as cantidad_peliculas 
      FROM listas l 
      LEFT JOIN lista_peliculas lb ON l.id = lb.lista_id 
      WHERE l.user_id = ? 
      GROUP BY l.id
    `;
    const dbName = await this.getDbName();
  ...
  async addMovieToLista(listaId: number, movie: Movie) {
    const sql = 'INSERT INTO lista_peliculas (lista_id, title, director, coverImage) VALUES (?, ?, ?, ?)';
    const dbName = await this.getDbName();
    return CapacitorSQLite.executeSet({
      database: dbName,
      set: [{
        statement: sql,
        values: [listaId, movie.title, movie.director, movie.coverImage]
      }]
    }).then((changes: capSQLiteChanges) => {
      if (this.isWeb) {
        CapacitorSQLite.saveToStore({ database: dbName });
  ...
  async getPeliculasLista(listaId: number) {
    const sql = 'SELECT * FROM lista_peliculas WHERE lista_id = ?';
    const dbName = await this.getDbName();
    return CapacitorSQLite.query({
      database: dbName,
      statement: sql,
  ...
  async deleteLista(listaId: number) {
    ...
    return CapacitorSQLite.executeSet({
      ...
      set: [
        {
          statement: 'DELETE FROM lista_peliculas WHERE lista_id = ?',
          values: [listaId]
        },
        {
  ...
  async removeMovieFromLista(listaId: number, movieId: number) {
    const sql = 'DELETE FROM lista_peliculas WHERE lista_id = ? AND id = ?';
    const dbName = await this.getDbName();
    return CapacitorSQLite.executeSet({
      database: dbName,
      set: [{
        statement: sql,
        values: [listaId, movieId]
      }]
    }).then((changes: capSQLiteChanges) => {
      if (this.isWeb) {
        CapacitorSQLite.saveToStore({ database: dbName });
